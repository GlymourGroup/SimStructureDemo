function(matchQualOptions=NULL){
  ...
  calculateMetrics <- function(bestMatches,
                measuredData){
    diffData <- data.frame(C=rep(NA,nrow(bestMatches)),
                           M=rep(NA,nrow(bestMatches)))
    ...
    diffData$C = combinedData$older_C - combinedData$younger_C
    diffData$M = combinedData$older_M - combinedData$younger_M
    diffData$manhattan = diffData$C + diffData$M
    diffData$euclidian = diffData$C^2 + diffData$M^2
    return(diffData)
  }
